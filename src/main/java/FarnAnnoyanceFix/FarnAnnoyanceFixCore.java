...
import FarnAnnoyanceFix.game.block.slab.BlockStepUpper;
import FarnAnnoyanceFix.game.block.slab.ItemSlabProxy;
...
public class FarnAnnoyanceFixCore {
	...
	public  void lateInit() {
		...
		/*if(mod_FarnAnnoyanceFix.slabplacement) {
			ModLoader.RegisterBlock(upperSlab = (new BlockStepUpper(mod_FarnAnnoyanceFix.upperslabid)).setResistance(10.0F).setStepSound(Block.soundStoneFootstep).setBlockName("stoneSlab"));
			FarnAnnoyanceFixCore.instance.addEffectiveTools(new Item[]{Item.pickaxeDiamond, Item.pickaxeGold, Item.pickaxeSteel, Item.pickaxeStone, Item.pickaxeWood}, new Block[]{upperSlab});
		...
		if(mod_FarnAnnoyanceFix.doubleslabsrecipe) {
		...
		if(mod_FarnAnnoyanceFix.doublestairsrecipe) {
