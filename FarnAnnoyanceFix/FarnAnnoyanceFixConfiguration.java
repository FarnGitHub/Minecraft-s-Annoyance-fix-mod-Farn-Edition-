	...
	public final void AddAllToolEffective() {
		...
		if (!configFile.exists()) {
			try {
				...
				this.WriteLine(configWriter, " ");
				for(int enumType1 = 0; enumType1 < EnumOptionsAnnoyanceFix.values().length; ++enumType1) {
					EnumOptionsAnnoyanceFix[] enumAF = EnumOptionsAnnoyanceFix.values();
					this.WriteLine(configWriter, "#" + enumAF[enumType1].enumDesc);
					this.WriteLine(configWriter, enumAF[enumType1].enumName + "=" + enumAF[enumType1].enumBoolean);
					this.WriteLine(configWriter, " ");
				}
				configWriter.close();
			} catch (Exception e) {
	...
	private void parseConfigTest(File var0) {
		try {
			...
			while((string2 = var1.readLine()) != null) {
				...
				for(int enumType1 = 0; enumType1 < EnumOptionsAnnoyanceFix.values().length; ++enumType1) {
					EnumOptionsAnnoyanceFix[] enumAF = EnumOptionsAnnoyanceFix.values();
					if(var3[0].equals(enumAF[enumType1].enumName)) {
						enumAF[enumType1].enumBoolean = var3[1].equals("true");
					}
				}
